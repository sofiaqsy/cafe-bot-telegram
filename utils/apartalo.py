...
def agregar_stock_pergamino(cantidad: float, motivo: str = "") -> bool:
    """
    Add kg to Pergamino Finca Rosal stock in apartalo-core.
    Called every time a PERGAMINO compra is saved.
    """
    try:
        payload = {
            "cantidad": int(cantidad),   # apartalo-core expects integer
            "operacion": "agregar",
            "motivo": motivo or f"Compra registrada via bot cafe ({cantidad} kg)",
        }
        resp = requests.post(
            _url(f"productos/{BUSINESS_ID}/{PROD_PERGAMINO}/stock"),
            json=payload,
            headers=_HEADERS,
            timeout=_TIMEOUT,
        )
        ...
        if resp.status_code == 200 and data.get("success"):
            logger.info(
                f"[APARTALO] Stock Pergamino actualizado: "
                f"{data.get('stockAnterior')} → {data.get('stockNuevo')} kg"
            )
            return True
        else:
            logger.warning(f"[APARTALO] Stock no actualizado: {data}")
            return False
    except Exception as e:
        logger.error(f"[APARTALO] Error actualizando stock: {e}")
        return False


def actualizar_precios(precio_pergamino: float, precio_oro_verde: float) -> bool:
    """
    Update prices of Pergamino and Verde Finca Rosal in apartalo-core.
