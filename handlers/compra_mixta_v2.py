"""
Implementación completamente corregida para el módulo de compra_mixta.

Esta corrección aborda múltiples problemas:
1. Conversión correcta del adelanto_id a entero
2. Manejo adecuado del saldo de adelanto
3. Eliminación de dobles barras invertidas en los mensajes
4. Importación directa de update_cell para evitar problemas de referencia
"""
...
# Importaciones directas para mejorar la fiabilidad
from utils.sheets import append_data as append_sheets
from utils.sheets import generate_unique_id, get_all_data, get_filtered_data
        ...
        if respuesta in ['sí', 'si', 's', 'yes', 'y', '✅ confirmar', 'confirmar']:
            try:
                ...
                if datos.get("monto_adelanto", 0) > 0 and datos.get("adelanto_id", ""):
                    try:
                        debug_log(f"Iniciando actualización de adelanto: {datos['adelanto_id']}")
                        ...
                        try:
                            ...
                            debug_log(f"Llamando a update_cell con: sheet_name='adelantos', row_index={adelanto_id_int}, column_name='saldo_restante', value={nuevo_saldo_formateado}")
                            result_adelanto = update_cell("adelantos", adelanto_id_int, "saldo_restante", nuevo_saldo_formateado)
                            
                            if result_adelanto:
                                debug_log(f"✅ Actualización exitosa del saldo de adelanto a {nuevo_saldo_formateado}")
                                mensaje_adelanto = f"✅ Saldo de adelanto actualizado correctamente a {formatear_precio(nuevo_saldo_formateado)}\n\n"
                            else:
                                debug_log("❌ La función update_cell devolvió False")
                                mensaje_adelanto = "⚠️ No se pudo actualizar el saldo de adelanto\n\n"
                        except (ValueError, TypeError) as e:
                            debug_log(f"Error al convertir adelanto_id a entero: {e}")
                            mensaje_adelanto = "⚠️ Error al actualizar saldo de adelanto (ID inválido)\n\n"
...
async def seleccionar_adelanto(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        for adelanto in adelantos:
            ...
            # Debugging de tipo y valor de adelanto_id
            debug_log(f"Adelanto ID en seleccionar_adelanto: {adelanto_id} - Tipo: {type(adelanto_id)}")
...
async def seleccionar_adelanto_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Debug del valor y tipo de adelanto_id
        debug_log(f"Adelanto ID extraído del callback: {adelanto_id} - Tipo: {type(adelanto_id)}")
        ...
        for adelanto in adelantos:
            ...
            debug_log(f"Comparando adelanto_id {adelanto_id} con row_index {row_index} - Tipos: {type(adelanto_id)} vs {type(row_index)}")
            
            # Convertir ambos a string para comparación
            if str(row_index) == str(adelanto_id):
                adelanto_seleccionado = adelanto
                debug_log(f"Adelanto encontrado: {adelanto}")
                break
